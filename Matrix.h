...
void appendRow(Matrix*,float);
int** getIntNumbers(Matrix);
void replaceMatrixNumber(Matrix*,float,float);
...
void appendRow(Matrix* matrix,float number){
    float* newRow = (float*)malloc(sizeof(float)*(matrix[0].columns));
    for(int i=0;i<matrix[0].columns;i++){
        newRow[i]=number;
    }
    matrix[0].numbers = (float**)realloc(matrix[0].numbers,sizeof(float**)*(matrix[0].rows+1));
    matrix[0].numbers[matrix[0].rows]=newRow;
    matrix[0].rows+=1;
}
int** getIntNumbers(Matrix a){
    int** intNumbers = (int**)malloc(sizeof(int*)*a.rows);
    for(int i=0;i<a.rows;i++){
        intNumbers[i]=(int*)malloc(sizeof(int*)*a.columns);
    }
    for(int i=0;i<a.rows;i++){
        for(int j=0;j<a.columns;j++){
            intNumbers[i][j]=(int)a.numbers[i][j];
        }
    }
    return intNumbers;
}
void replaceMatrixNumber(Matrix* a,float replace,float number){
    for(int i=0;i<a[0].rows;i++){
        for(int j=0;j<a[0].columns;j++){
            if(a[0].numbers[i][j]==replace){
                a[0].numbers[i][j]=number;
            }
        }
    }
}